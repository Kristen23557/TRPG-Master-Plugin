...
pending_checks = {}   # {check_id: check_data}
...
class CheckSystem:
    ...
    @staticmethod
    async def create_pending_check(check_data: Dict) -> str:
        """创建待处理检定"""
        check_id = generate_check_id()
        check_data['check_id'] = check_id
        check_data['created_time'] = datetime.now().isoformat()
        check_data['expire_time'] = (datetime.now() + timedelta(minutes=1)).isoformat()
        
        pending_checks[check_id] = check_data
        
        # 启动超时检查
        asyncio.create_task(CheckSystem._check_timeout(check_id))
        
        return check_id
    
    @staticmethod
    async def _check_timeout(check_id: str):
        """检查检定超时"""
        await asyncio.sleep(60)  # 1分钟
        
        if check_id in pending_checks:
            check_data = pending_checks[check_id]
            await CheckSystem._auto_perform_check(check_data)
    
    @staticmethod
    async def _auto_perform_check(check_data: Dict):
        """自动执行检定"""
        check_id = check_data['check_id']
        
        if check_data.get('is_simulate'):
            # 模拟检定自动掷骰
            result = await CheckSystem._perform_simulate_check(check_data)
        else:
            # 正常检定自动掷骰
            result = await CheckSystem._perform_normal_check(check_data)
        
        # 发送结果
        from src.plugin_system.apis import send_api
        await send_api.send_text(
            check_data['stream_id'],
            f"⏰ **检定超时自动完成**\n{result}"
        )
    ...
    @staticmethod
    async def _perform_normal_check(check_data: Dict) -> str:
        """执行正常检定"""
        character = character_db.get(check_data['character_rid'])
        if not character:
            return "❌ 角色数据丢失"
        
        mode = check_data['mode']
        check_type = check_data['check_type']
        modifier = check_data.get('modifier', '')
        
        return await CheckSystem._perform_check_logic(check_type, modifier, character, mode)
    
    @staticmethod
    async def _perform_simulate_check(check_data: Dict) -> str:
        """执行模拟检定"""
        mode = check_data['mode']
        check_type = check_data['check_type']
        target_value = check_data['target_value']
        ...
        virtual_character = {
            "name": "模拟角色",
            "mode": mode,
            "attributes": {"simulate": target_value}
        }
        
        # 根据模式执行检定逻辑
        if mode == "coc":
            return await CheckSystem._perform_coc_check(
                f"{check_type}模拟检定", target_value, modifier, 
                virtual_character, "模拟属性"
            )
        else:
            return await CheckSystem._perform_dnd_check(
                f"{check_type}模拟检定", difficulty, modifier,
                virtual_character, "模拟属性"
            )
    
    @staticmethod
    async def _perform_check_logic(check_type: str, modifier: str, character: Dict, mode: str) -> str:
        """执行检定逻辑"""
        ...
        if check_type in attributes:
            ...
            if mode == "coc":
                ...
                base_attribute = CheckSystem._get_dnd_skill_attribute(check_type)
        ...
        if mode == "coc":
            return await CheckSystem._perform_coc_check(check_name, target_value, modifier, character, attribute_used)
        else:
            return await CheckSystem._perform_dnd_check(check_name, target_value, modifier, character, attribute_used)
    
    @staticmethod
    def _get_dnd_skill_attribute(skill: str) -> str:
        """获取DnD技能对应的主要属性"""
        skill_attributes = {
            "运动": "力量",
            "潜行": "敏捷", "巧手": "敏捷",
    ...
    @staticmethod
    async def _perform_coc_check(check_name: str, target_value: int, modifier: str, character: Dict, attribute_used: str) -> str:
        """执行CoC检定"""
        roll_details = ""
        if modifier == "adv":
    ...
    @staticmethod
    async def _perform_dnd_check(check_name: str, target_value: int, modifier: str, character: Dict, attribute_used: str) -> str:
        """执行DnD检定"""
        roll_details = ""
        if modifier == "adv":
        ...
        success = roll_result >= target_value
...
class PlotAdvancer:
    ...
    @staticmethod
    async def advance_plot(session_id: str, player_action: str = "") -> str:
        """推进剧情，考虑玩家行为"""
        if session_id not in active_sessions:
            return "❌ 会话不存在"
        
        session = active_sessions[session_id]
        plot_name = session["plot_name"]
        
        # 加载剧本内容
        plot_content = await load_plot_content(plot_name)
        if not plot_content:
            return f"❌ 剧本 '{plot_name}' 不存在或加载失败"
        
        # 获取当前进度
        current_progress = session.get("plot_progress", 0)
        
        # 分析玩家行为并生成下一段剧情
        next_plot_text = await PlotAdvancer._generate_next_plot(
            plot_content, current_progress, player_action, session
        )
        
        # 更新进度
        session["plot_progress"] = current_progress + 1
        session["last_activity"] = datetime.now().isoformat()
        
        # 生成操作建议
        suggestions = await PlotAdvancer._generate_suggestions(next_plot_text, session)
        ...
        response += f"📊 进度: {session['plot_progress'] + 1}/未知"
        
        return response
    
    @staticmethod
    async def _generate_next_plot(plot_content: str, current_progress: int, 
                                player_action: str, session: Dict) -> str:
        """生成下一段剧情文本"""
        # 分割剧本内容
        plot_segments = plot_content.split('\n\n')
        
        # 如果有玩家行为，结合行为生成剧情
        if player_action:
            return await PlotAdvancer._generate_plot_with_action(
                plot_segments, current_progress, player_action, session
            )
        else:
            # 无行为时按顺序推进
            if current_progress < len(plot_segments):
                return plot_segments[current_progress]
            else:
                return "🎭 剧本已结束。感谢参与！"
    
    @staticmethod
    async def _generate_plot_with_action(plot_segments: List[str], current_progress: int,
                                       player_action: str, session: Dict) -> str:
        """结合玩家行为生成剧情"""
        # 简化的剧情生成逻辑
        base_plot = plot_segments[current_progress] if current_progress < len(plot_segments) else ""
        
        # 根据玩家行为调整剧情
        action_impact = await PlotAdvancer._analyze_action_impact(player_action, session)
        
        if action_impact == "positive":
            modifier = "\n\n💫 你的行动带来了积极的结果！"
        elif action_impact == "negative":
            modifier = "\n\n⚠️ 你的行动带来了意外的后果..."
        elif action_impact == "creative":
            modifier = "\n\n✨ 你的创意行动改变了局面！"
        else:
            modifier = "\n\n📝 你的行动影响了故事的发展。"
        
        return f"{base_plot}\n\n**你的行动:** {player_action}{modifier}"
    
    @staticmethod
    async def _analyze_action_impact(player_action: str, session: Dict) -> str:
        """分析玩家行为的影响"""
        action_lower = player_action.lower()
        
        # 简单关键词分析
        positive_keywords = ["帮助", "治疗", "保护", "合作", "调查", "寻找", "询问", "探索"]
        negative_keywords = ["攻击", "破坏", "偷窃", "欺骗", "威胁", "逃跑", "忽视"]
        creative_keywords = ["创造", "发明", "组合", "使用", "尝试", "推理", "解决"]
    ...
    @staticmethod
    async def _generate_suggestions(plot_text: str, session: Dict) -> str:
        """根据剧情生成操作建议"""
        suggestions = []
        plot_lower = plot_text.lower()
        
        # 基于剧情内容生成建议
        if any(word in plot_lower for word in ["战斗", "敌人", "攻击"]):
            suggestions.extend(["⚔️ 准备战斗", "🛡️ 采取防御姿态", "🔍 寻找掩护"])
        
        if any(word in plot_lower for word in ["谜题", "谜团", "秘密"]):
            suggestions.extend(["🔎 调查线索", "💭 思考推理", "🗣️ 讨论分析"])
        
        if any(word in plot_lower for word in ["社交", "对话", "交谈"]):
            suggestions.extend(["🗣️ 进行对话", "🎭 角色扮演", "🤝 建立关系"])
        
        if any(word in plot_lower for word in ["探索", "地点", "房间"]):
            suggestions.extend(["🔍 搜索环境", "📝 记录发现", "🗺️ 绘制地图"])
        
        if any(word in plot_lower for word in ["危险", "陷阱", "威胁"]):
            suggestions.extend(["⚠️ 谨慎前进", "🔍 侦查危险", "🎯 准备检定"])
    ...
    async def execute(self) -> Tuple[bool, Optional[str], bool]:
        try:
            ...
            active_sessions[session_id] = {
                ...
                "plot_progress": save_data.get("plot_progress", 0),
                "created_time": datetime.now().isoformat(),
                "last_activity": datetime.now().isoformat(),
...
class SaveCommand(BaseCommand):
    ...
    async def _save_game(self, user_id: str) -> Tuple[bool, Optional[str], bool]:
        ...
        save_data = {
            ...
            "plot_progress": current_session.get("plot_progress", 0),
...
class ActionCommand(BaseCommand):
    ...
    async def execute(self) -> Tuple[bool, Optional[str], bool]:
        try:
            user_id = self.message.message_info.user_info.user_id
            action_text = self.matched_groups.get("action_text", "").strip()
            
            # 处理help参数
            if action_text == "help" or not action_text:
                return await self._show_help()
            ...
            await self.send_text(f"❌ 行动执行失败: {str(e)}")
            return False, f"行动失败: {str(e)}", True
...
class CheckCommand(BaseCommand):
    ...
    command_pattern = r"^/check\s+(?P<check_type>\S+)(?:\s+(?P<modifier>adv|dis|help|simulate))?(?:\s+(?P<params>.+))?$"
    intercept_message = True
    ...
    async def execute(self) -> Tuple[bool, Optional[str], bool]:
        try:
            ...
            # 检查是否为模拟检定
            if modifier == "simulate":
                return await self._handle_simulate_check(user_id, check_type, params)
            else:
                return await self._handle_normal_check(user_id, check_type, modifier)
                
        except Exception as e:
            await self.send_text(f"❌ 检定失败: {str(e)}")
            return False, f"检定失败: {str(e)}", True
    
    async def _handle_normal_check(self, user_id: str, check_type: str, modifier: str) -> Tuple[bool, Optional[str], bool]:
        """处理正常检定"""
        # 检查用户注册
        registered, msg = check_user_registered(user_id)
        if not registered:
        ...
        # 显示检定详情
        check_details = await self._get_check_details(check_type, modifier, character, current_session["mode"])
        await self.send_text(check_details)
        
        # 创建待处理检定
        chat_stream = getattr(self, 'chat_stream', None)
        stream_id = getattr(chat_stream, 'stream_id', 'unknown') if chat_stream else 'unknown'
        ...
        check_id = await CheckSystem.create_pending_check(check_data)
        
        # 提示玩家进行掷骰
        await self.send_text(
            f"⏰ **检定已准备就绪** (ID: {check_id})\n"
            f"请在1分钟内使用以下命令进行掷骰：\n"
            f"`/dice check {check_id}`\n\n"
            f"💡 提示：超时将自动掷骰"
        )
        
        return True, "检定准备完成", True
    
    async def _handle_simulate_check(self, user_id: str, check_type: str, params: str) -> Tuple[bool, Optional[str], bool]:
        """处理模拟检定"""
        # 解析参数: 属性值 难度
        pattern = r'^(\d+)(?:\s+(\d+))?$'
        match = re.match(pattern, params)
        ...
        if not match:
            await self.send_text("❌ 模拟检定参数格式错误！使用: /check <类型> simulate <属性值> [难度]")
            return False, "参数格式错误", True
        
        target_value = int(match.group(1))
        difficulty = int(match.group(2)) if match.group(2) else 10
        
        # 确定模式（简化处理，使用coc作为默认）
        mode = "coc" if target_value > 20 else "dnd"
        
        # 显示模拟检定详情
        check_name = f"{check_type}模拟检定"
        if mode == "coc":
            check_info = (
                f"🎲 **{check_name}** (CoC模拟)\n"
                f"📊 属性值: **{target_value}**\n"
            ...
            check_info = (
                f"🎲 **{check_name}** (DnD模拟)\n"
                f"📊 属性调整值: **{target_value}**\n"
                f"📈 难度等级(DC): **{difficulty}**\n"
                f"📋 DnD规则: 骰值 ≥ DC 即为成功"
            )
        
        await self.send_text(check_info)
        ...
        # 提示玩家进行掷骰
        await self.send_text(
            f"⏰ **模拟检定已准备就绪** (ID: {check_id})\n"
            f"请在1分钟内使用以下命令进行掷骰：\n"
            f"`/dice check {check_id}`\n\n"
            f"💡 提示：超时将自动掷骰"
        )
        
        return True, "模拟检定准备完成", True
    
    async def _get_check_details(self, check_type: str, modifier: str, character: Dict, mode: str) -> str:
        """获取检定详情"""
        attributes = character["attributes"]
        ...
        modifier_text = ""
        if modifier == "adv":
            modifier_text = "🎯 **优势检定** (取较好结果)"
        elif modifier == "dis":
            modifier_text = "⚠️ **劣势检定** (取较差结果)"
...
**正常检定:**
`/check <检定类型> [adv|dis]`
示例: `/check 侦查` 或 `/check 力量检定 adv`

**模拟检定:**
`/check <检定类型> simulate <属性值> [难度]`
示例: `/check 侦查 simulate 60` (CoC)
示例: `/check 力量检定 simulate 15 12` (DnD)

**检定流程:**
1. 使用 `/check` 设置检定参数
2. 在1分钟内使用 `/dice check <检定ID>` 进行掷骰
3. 超时将自动掷骰
...
class DiceCommand(BaseCommand):
    ...
    command_pattern = r"^/dice\s+(?P<subcommand>\w+)(?:\s+(?P<params>.+))?$"
    intercept_message = True
    
    async def execute(self) -> Tuple[bool, Optional[str], bool]:
        try:
            subcommand = self.matched_groups.get("subcommand", "")
            params = self.matched_groups.get("params", "")
            
            if subcommand == "check":
                return await self._handle_dice_check(params)
            elif subcommand == "help":
                return await self._show_help()
            else:
                # 处理普通骰子
                return await self._handle_normal_dice(subcommand, params)
                
        except Exception as e:
            await self.send_text(f"❌ 掷骰失败: {str(e)}")
            return False, f"掷骰失败: {str(e)}", True
    
    async def _handle_dice_check(self, params: str) -> Tuple[bool, Optional[str], bool]:
        """处理检定掷骰"""
        if not params:
            await self.send_text("❌ 请提供检定ID，使用: `/dice check <检定ID>`")
            return False, "缺少检定ID", True
        
        check_id = params.strip()
        if check_id not in pending_checks:
            await self.send_text("❌ 检定ID无效或已过期")
            return False, "无效检定ID", True
        
        check_data = pending_checks[check_id]
        user_id = self.message.message_info.user_info.user_id
        
        # 检查权限
        if str(user_id) != str(check_data['user_id']):
            await self.send_text("❌ 您不能执行其他人的检定")
            return False, "权限不足", True
    ...
    async def _handle_normal_dice(self, dice_type: str, params: str) -> Tuple[bool, Optional[str], bool]:
        """处理普通骰子"""
        # 解析骰子类型，如 D20, 2D6, D100 等
        pattern = r'^D(?P<sides>\d+)$'
        match = re.match(pattern, dice_type)
        
        if not match:
            await self.send_text("❌ 骰子格式错误！使用: D<面数> 或 数量D<面数>")
            return False, "骰子格式错误", True
        ...
        if params:
            try:
                ...
                if count < 1 or count > 10:
                    await self.send_text("❌ 骰子数量必须在1-10之间")
                    return False, "数量无效", True
            except ValueError:
                await self.send_text("❌ 骰子数量必须是数字")
        ...
        if count == 1:
            await self.send_text(f"🎲 掷出了 D{sides}: **{results[0]}**")
        else:
            rolls_text = " + ".join(map(str, results))
            await self.send_text(f"🎲 掷出了 {count}D{sides}: {rolls_text} = **{total}**")
        
        return True, f"掷骰结果: {total}", True
